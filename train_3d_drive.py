...
parser.add_argument('--train_log_path', default='/data/lecui/3d_drive/log/pointSIFT_train')
parser.add_argument('--test_log_path', default='/data/lecui/3d_drive/log/pointSIFT_test')
...
class SegTrainer(object):
    ...
    def t_net_dirct_choose(self, point_set, semantic_seg):
            ...
            print("choice contain semantic_seg label : ", np.unique(semantic_seg))
            mask = mask[choice]
    ...
    def t_net_whole(self, all_point_set, all_semantic_seg):
        ...
        bound_x = 50
        bound_y = 50 
        bound_z = 4.5 # box max height

        #searching stride
        stride = 5 # stride = box_x  -- means no overlapping
        box_x = 5
        box_y = 5
