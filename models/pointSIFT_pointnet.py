...
def get_loss(pred, label, smpw):
    ...
    # print("classify_loss shape: ", classify_loss.shape)
    tf.summary.scalar('classify loss', classify_loss)
    tf.add_to_collection('losses', classify_loss)
    return classify_loss

def get_focal_loss(pred, label, gamma = 2, alpha = 0.25):
    """
    :param pred: BxNxC
    :param label: BxN
    :param smpw: BxN
    :return:
    """
    y_pred = tf.nn.softmax(pred, dim=-1)
    label = tf.one_hot(label, depth=y_pred.shape[2])
    classify_loss = alpha * (-label*((1-y_pred)**gamma)*tf.log(y_pred))
    classify_loss = tf.reduce_sum(classify_loss, axis = None)
    print("classify_loss : ", classify_loss)
    print("classify_loss shape: ", classify_loss.shape)
    tf.summary.scalar('classify loss', classify_loss)
    tf.add_to_collection('losses', classify_loss)
...
def get_focal_loss_2(pred, label, gamma = 2.0, alpha = 4.0):
    """
    :param pred: BxNxC
    :param label: BxN
    :param smpw: BxN
