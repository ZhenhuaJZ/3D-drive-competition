...
MODEL_PATH = os.path.join(ROOT, 'Models_2/10_10_18/best_seg_model_15.ckpt') # choose the models
...
def test(save_mode = False):
    ...
    #read csv path
        ...
        for i in tqdm(range(len(txt))):
            ...
            # lidar_inbound = np.array(lidar_inbound)
            ...
            step_list = []
            curmin_max_list = []
            # lidar_inbound = lidar_inbound.values
            for _x in range(nsubvolume_x):
                for _y in range(nsubvolume_y):
                    ...
                    # print(lidar_inbound.astype('float64'))
                    # print(type(lidar_inbound.iloc[0,0]))
                    # lidar_inbound lidar_inbound.astype('float16')
                    # print(curmin.shape)
                    
                    curchoice = np.sum((lidar_inbound >= (curmin)) & (lidar_inbound <= (curmax)), axis=1) == 3

                    # find_true = (lidar_inbound.loc[9982].values>=curmin) & (lidar_inbound.loc[9982].values<=curmax)
                    # print(find_true)
                    # if np.sum(np.array(find_true).reshape(1,3) ,axis = 1) > 1:
                        # true_list.append([np.array(find_true).reshape(1,3), _x, _y, curmin, curmax])
                    # if np.sum(curchoice) > 0: #box has point
                    #     cur_point_set = lidar_inbound.iloc[np.array(curchoice)]
                        # points_list.append(cur_point_set.index.tolist())
            ...
            # print("final :" , pd.concat([inbound_pred, outbound_pred]).sort_index())
            # print("final len :" , len(pd.concat([inbound_pred, outbound_pred]).sort_index()))
